...
static JsonDict toJson(const QTorrentHandle& h)
{
  ...
  ret.add(KEY_TORRENT_RATIO, (ratio > 100.) ? "¡Þ" : QString::number(ratio, 'f', 1));
  QString eta;
  QString state;
  if (h.is_paused()) {
    if (h.has_error())
  ...
  ret.add(KEY_TORRENT_ETA, eta.isEmpty() ? "¡Þ" : eta);
...
QString btjson::getPropertiesForTorrent(const QString& hash)
{
  ...
  try {
    ...
    const qreal ratio = QBtSession::instance()->getRealRatio(h.hash());
    data.add(KEY_PROP_RATIO, ratio > 100. ? tr("¡Þ") : QString::number(ratio, 'f', 1));
  } catch(const std::exception& e) {
    qWarning() << Q_FUNC_INFO << "Invalid torrent: " << e.what();
    return QString();
  }
